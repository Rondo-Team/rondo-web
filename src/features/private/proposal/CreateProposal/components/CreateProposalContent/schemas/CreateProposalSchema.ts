...
export const CreateProposalSchema = (t: (arg: string) => string) =>
  z.object({
    title: z
      .string({ error: t("title.empty") })
      .trim()
      .min(CREATE_PROPOSAL_LIMITS.title.min, { error: t("title.tooShort") })
      .max(CREATE_PROPOSAL_LIMITS.title.max, { error: t("title.tooLong") }),
    description: z
      .string({ error: t("description.empty") })
      .trim()
      .min(CREATE_PROPOSAL_LIMITS.description.min, {
        error: t("description.tooShort"),
      })
      .max(CREATE_PROPOSAL_LIMITS.description.max, {
...
type CreateProposalSchemaType = ReturnType<typeof CreateProposalSchema>;
export type CreateProposalActionDTO = z.infer<CreateProposalSchemaType>;
